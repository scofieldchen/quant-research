...
def _(calculate_features, pd):
    ...
    df
    return df, features, ohlcv, target
...
def _(features, mo, pd):
    feature_name = mo.ui.dropdown.from_series(
        pd.Series(features.columns), value=features.columns[0]
    )
    return (feature_name,)
...
def _(
    ...
    feature_dist_fig, feature_dist_ax = visualize_distribution(
    ...
    mo.vstack(
        [
            feature_name,
            mo.md("---"),
            mo.hstack([feature_stats, feature_dist_ax], widths=[1, 3]),
        ]
    )
...
def _(List, go, np, pd):
    def plot_correlation_heatmap(
        ...
        fig.update_layout(
            ...
            title="Interactive Correlation Heatmap of Selected Features",
...
@app.cell
def _(features, mo, pd):
    feature_name_2 = mo.ui.dropdown.from_series(
        pd.Series(features.columns), value=features.columns[0]
    )
...
def _(df, feature_name_2, mo, plot_single_violin):
    ...
    mo.vstack([feature_name_2, mo.md("---"), ax_violin])
...
def _(Any, Dict, NDArray, kruskal, np):
    def perform_kruskal_wallis(
        feature: NDArray, target: NDArray
    ) -> Dict[str, Any]:
        """
...
def _(df, perform_kruskal_wallis):
    ...
    for col in df.drop(columns="target").columns:
        ...
        if pvalue < alpha:
            print(
                f"{col}: p-value({pvalue:.2%}) < {alpha:.1%}. The null hypothesis is REJECTED, significant difference in the median of the feature across different categories."
            )
        else:
            print(
                f"{col}: p-value({pvalue:.2%}) > {alpha:.1%}. The null hypothesis is ACCEPTED, no significant difference in the median of the feature across different categories."
            )
